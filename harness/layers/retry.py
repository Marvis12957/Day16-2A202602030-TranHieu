...
class Retry(Middleware):
    ...
    def wrap_tool_call(self, ctx, call, name, args):
        ...
        attempts = 1
        # Tầng công cụ khoá xác suất hỏng theo (seed, chỉ số lượt gọi),
        # nên lượt gọi lại rơi vào chỉ số MỚI và được tung lại độc lập.
        while attempts < self.max_attempts and self._broken(result):
            if not self._budget_left(ctx):
                break  # đừng tiêu lượt dành cho `submit`
            result = call(name, args)
            attempts += 1
        self.bump(ctx, "retries", attempts - 1)
        if self._broken(result):
            self.bump(ctx, "gave_up")
        # Trả về sự thật, kể cả khi vẫn hỏng.
        return result
