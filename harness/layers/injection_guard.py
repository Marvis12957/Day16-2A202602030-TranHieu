...
class InjectionGuard(Middleware):
    ...
    def wrap_tool_call(self, ctx, call, name, args):
        ...
        if BLOCK_START not in content:
            return result
        # Một quan sát có thể mang nhiều đoạn độc; cắt cho tới khi hết.
        while True:
            start = content.find(BLOCK_START)
            if start < 0:
                break
            end = content.find(BLOCK_END, start)
            # Fetch bị cắt giữa chừng: có dấu mốc MỞ mà không có dấu mốc
            # ĐÓNG. Vẫn phải cắt, tới hết chuỗi.
            tail = "" if end < 0 else content[end + len(BLOCK_END):]
            content = content[:start] + PLACEHOLDER + tail
        self.bump(ctx, "blocks_removed")
